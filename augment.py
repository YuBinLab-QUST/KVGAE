...
# Calculate the cosine similarity between two sets of data
...
def find_adjacent_spot(
    adata, use_data="raw", neighbour_k=6, verbose=False
):
    if use_data == "raw":
    ...
    with tqdm(
        total=len(adata),
        desc="Find adjacent spots of each spot",
        ...
        for i in range(adata.shape[0]):
            ...
            if spot_weight.sum() > 0:
                ...
                spot_matrix_scaled = np.multiply(
                    spot_weight_scaled.reshape(-1, 1), spot_matrix
                )
...
def augment_gene_data(adata, adjacent_weight=0.2):
    if isinstance(adata.X, np.ndarray):
        augment_gene_matrix = adata.X + adjacent_weight * adata.obsm[
            "adjacent_data"
        ].astype(float)
    elif isinstance(adata.X, csr_matrix):
        augment_gene_matrix = adata.X.toarray() + adjacent_weight * adata.obsm[
            "adjacent_data"
        ].astype(float)
...
def augment_adata(
    ...
    adata = find_adjacent_spot(
        adata, use_data=use_data, neighbour_k=neighbour_k
    )
    adata = augment_gene_data(adata, adjacent_weight=adjacent_weight)
    return adata